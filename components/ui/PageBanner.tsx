...
const PageBanner = ({
  ...
  return (
    <section className="relative h-90 md:h-120 pt-20 overflow-hidden w-full">
      ...
      <div
        className="absolute pt-20 top-0 left-0 w-full h-full 
                        bg-[rgba(0,0,0,0.2)] flex items-center 
                        justify-center z-[2] flex-col gap-2"
      >
        <h1 className="font-bold text-3xl md:text-6xl text-white text-shadow-lg">{title}</h1>
        {children}
      </div>
    </section>
  );
...
const Breadcrumb = ({ breadcrumb }: { breadcrumb: { title: string; path?: string }[] }) => {
...
const Category = ({
  ...
  return (
    <nav className="w-full flex justify-center items-center p-4 gap-8 border-b border-gray-300">
      {category.map((item) => (
        <Link
          className={`${select === item.path ? 'font-bold' : ''}`}
          key={item.title}
          href={`${root}${item.path}`}
        >
          {item.title}
